...
- **CI wiring (`build.yml`)** — added a `resolve` job that queries the same JetBrains releases feed `compatibility-verification.yml` already used, but collapses it to a single target: prefer the latest **stable release** within `pluginSinceBuild..pluginUntilBuild`, falling back to the latest **EAP** when no stable release exists yet. `build`, `test`, and `verify` each depend on `resolve`, download+extract that one IDE, and pass `-PlocalRiderPath=<path>` to every Gradle invocation in the job (including the `properties`/`getChangelog` calls in the `build` job's "Export Properties" step — see Deviations below for why that step needed the flag too). This design self-heals once Rider 2026.2 goes GA: the `resolve` job will start returning the stable release build automatically, no workflow edit required.
- **`compatibility-verification.yml`'s `verify` job** was also updated to pass `-PlocalRiderPath` (in addition to the pre-existing `-PverifyIdePaths`), reusing the same downloaded IDE directory for both — see Deviations for why this was necessary.
...
- **CI change ended up broader than "add a 262 build/verify path"** as originally scoped in the plan. Empirically confirmed (by running `./gradlew compileKotlin` locally without `-PlocalRiderPath`) that the `create("RD", providers.gradleProperty("platformVersion").get())` fallback in `build.gradle.kts` fails at **Gradle configuration time**, not just for `buildPlugin`/`verifyPlugin` — because `pluginVerification.ides`'s fallback branch (`local(intellijPlatform.platformPath.toFile())`) eagerly evaluates `platformPath` while the `intellijPlatform {}` block is being configured, regardless of which task is ultimately run. This meant every job in `build.yml` (including `build`'s "Export Properties" step, which only calls `properties`/`getChangelog`) needed `-PlocalRiderPath`, and `compatibility-verification.yml`'s `verify` job needed it too (previously it only passed `-PverifyIdePaths`, relying on the old stable `create("RD","2025.3.3")` compile target to succeed independently).

### Tradeoffs

- **Kept the `create(platformType, platformVersion)` fallback in `build.gradle.kts`** rather than replacing it with a fail-fast custom error, so that once Rider 2026.2 ships a stable release with a resolvable Maven coordinate, repository resolution can start working again with no code change — just update `platformVersion`. Accepted tradeoff: until then, a contributor who forgets `-PlocalRiderPath` locally sees a generic Gradle error (`No IntelliJ Platform dependency found`) rather than a project-specific message; documented the requirement and the error signature in `CLAUDE.md`'s Build section instead.
...
- **`build.yml`'s CI resolution targets exactly one IDE build** (latest release-or-EAP in range) rather than the full release+EAP matrix `compatibility-verification.yml` runs. This keeps day-to-day PR/push CI fast and stable (not broken by every new unrelated EAP build), while the periodic/manual `compatibility-verification.yml` remains the mechanism for catching future breaking changes across the full Marketplace-verified range.
- **Local SDK download for CI** (`curl` + `tar` extraction of the Linux artifact, matching `compatibility-verification.yml`'s existing pattern) was chosen over any form of dependency caching between jobs — reusing the existing, already-proven pattern was judged more reliable than optimizing away the redundant ~2.3GB download per job at this stage.
...
- Whether to eventually replace the `create()` repository-resolution fallback in `build.gradle.kts` with a fail-fast error (clearer message) once it's confirmed nobody relies on repository resolution working for Rider 2026.2, versus keeping it as auto-resolve-when-GA insurance.
- CI now downloads the ~2.3GB Rider archive once per job in `build.yml` (3 times per workflow run) plus once per matrix entry in `compatibility-verification.yml` — acceptable for now; worth revisiting with caching if CI minutes/bandwidth become a concern.
